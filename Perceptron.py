...
def treinar_perceptron(X, y, epocas=1000, taxa_aprendizado=.1):
...
matriz_max_acuracia = matriz_de_confusao(y[split_index:], modelos[indice_max_acuracia][1])
matriz_min_acuracia = matriz_de_confusao(y[split_index:], modelos[indice_min_acuracia][1])
...
plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
plt.plot(modelos[indice_max_acuracia][1], label='Predições - Maior Acurácia')
...
plt.subplot(1, 2, 2)
plt.plot(modelos[indice_min_acuracia][1], label='Predições - Menor Acurácia')
...
# Plotando boxplot para as métricas
